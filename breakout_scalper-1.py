...
# =============================================================================
#  PENDING SIGNALS — breakouts awaiting an M15 retest
# =============================================================================
# detect_h1_breakout() only flags a breakout on the exact H1 bar where the cross
# happens; the moment the next H1 bar closes, that bar stops being "last" and the
# function goes quiet on it. Without this file, the M15 retest would only ever get
# checked during the ~1 hour the breakout bar is still the most recent one, instead
# of the full max_retest_age window the config implies. This file persists each
# breakout so it keeps getting checked against fresh M15 data on every scan until
# it either retests or genuinely expires.
PENDING_FILE = "scalper_pending.json"
...
def save_pending(data):
    ...
    except Exception as e: log.error("Pending save: %s", e)
...
def detect_h1_breakout(h1):
    ...
    # Deriv's "end: latest" returns the still-forming candle as the last row.
    # Drop it so breakout detection only ever fires on a fully-closed H1 bar.
    last     = df.iloc[-2]
    prev     = df.iloc[-3]
...
def detect_m15_retest(m15, breakout):
    ...
    if breakout["breakout_bar"] is not None:
        breakout_close = breakout["breakout_bar"] + pd.Timedelta(seconds=CFG.h1_tf)
        df = df[df.index >= breakout_close]
...
async def scan_symbol(symbol):
    ...
    # --- Fresh H1 breakout on this exact bar: start tracking it if it qualifies ---
    if not direction:
        print(f"  H1: No breakout detected", flush=True)
    else:
        print(f"  H1: {direction} breakout at {level}  |  {rating} ({score}/6)", flush=True)

        aligned = (direction == "BULL" and h4_bias == "BULLISH") or \
                  (direction == "BEAR" and h4_bias == "BEARISH")

        if not aligned:
            print(f"  SKIPPED — H4 bias is {h4_bias} (needs alignment)", flush=True)
        elif adx_val < CFG.adx_min:
            ...
            if key not in pending:
                pending[key] = {
                    ...
                    "breakout_bar": breakout["breakout_bar"].isoformat(),
                    "score": score, "rating": rating, "h4_bias": h4_bias,
                    "atr": breakout["atr"], "first_seen": time.time(),
                }
                save_pending(pending)
                if not is_duplicate(symbol, direction + "_HU", level):
                    send_telegram(build_headsup(symbol, direction, level, now, h4_bias))
                    mark_sent(symbol, direction + "_HU", level)

    # --- Check every pending breakout for this symbol against fresh M15 data ---
    sym_keys = [k for k, v in pending.items() if v["symbol"] == symbol]
    if not sym_keys:
        return
    ...
    for key in sym_keys:
        entry = pending[key]
        b_bar = pd.Timestamp(entry["breakout_bar"])
        age_h = (now_utc - b_bar).total_seconds() / 3600

        if age_h > CFG.max_retest_age:
            print(f"  PENDING EXPIRED: {key}  (age {age_h:.1f}h)", flush=True)
            del pending[key]
            save_pending(pending)
            continue
        ...
        synth_breakout = {
            ...
            "broken_level": entry["level"],
            "breakout_bar": b_bar,
            "atr":          entry["atr"],
            "score":        entry["score"],
        }
        retest = detect_m15_retest(m15, synth_breakout)

        if retest.get("expired"):
            print(f"  PENDING EXPIRED (M15 window): {key}", flush=True)
            del pending[key]
            save_pending(pending)
            continue

        if not retest.get("retest"):
            print(f"  PENDING watching: {key}  (age {age_h:.1f}h)", flush=True)
            continue

        wick_ratio = retest.get("wick_ratio", 0)
        print(f"  PENDING RETEST CONFIRMED: {key}  wick={wick_ratio:.2f}", flush=True)

        trade = build_trade(synth_breakout, retest)
        if not trade:
            continue
        ...
        if not is_duplicate(symbol, entry["direction"], entry["level"]):
            msg = build_alert(
                symbol=symbol, direction=entry["direction"], broken_level=entry["level"],
                entry=trade["entry"], sl=trade["sl"], tp1=trade["tp1"], tp2=trade["tp2"],
                risk=trade["risk"], rr=trade["rr"],
                h1_time=b_bar, retest_time=retest["retest_bar"],
                score=entry["score"], rating=entry["rating"], h4_bias=entry["h4_bias"],
            )
            send_telegram(msg)
            mark_sent(symbol, entry["direction"], entry["level"])
        else:
